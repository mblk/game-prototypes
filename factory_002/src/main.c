...
int main(void) {
    ...
    game_state_t game_state = {}; reset_game_state(&game_state);
    ...
    // -----------------
    build_some_stuff(&game_state);
    build_some_more_stuff(&game_state);
    // -----------------
    ...
    while(!WindowShouldClose()) {
        ...
        if (!mouse_is_over_ui) {
            ...
            if (IsMouseButtonDown(MOUSE_BUTTON_LEFT)) {
                ...
                if (clicked_building && selected_building && clicked_building != selected_building) {
                    connect_buildings(&game_state, selected_building, clicked_building);
                }
                if (clicked_building) {
                    ...
                    switch (building_recipe) {
                        case 1: new_building = spawn_miner(&game_state, mouse_coord); break;
                        case 2: new_building = spawn_belt(&game_state, mouse_coord); break;
                        case 3: new_building = spawn_factory(&game_state, mouse_coord); break;
                    }
                    if (selected_building && new_building) {
                        connect_buildings(&game_state, selected_building, new_building);
                    }
        ...
        BeginDrawing();
        {
            ...
            BeginMode2D(camera);
            {
                ...
                if (selected_building != 0) {
                    building_t *sb = game_state.buildings + selected_building;
            ...
            if (selected_building != 0) {
                DrawText(TextFormat("sel: %u", selected_building), 10, next_text_y+=20, 20, WHITE);
            }
        }
        EndDrawing();
    }
